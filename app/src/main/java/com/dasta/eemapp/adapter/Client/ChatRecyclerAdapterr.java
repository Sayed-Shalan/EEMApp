...
public class ChatRecyclerAdapterr extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final int VIEW_TYPE_ME = 1;
    private static final int VIEW_TYPE_OTHER = 2;

    private List<Chat> mChats;

    public ChatRecyclerAdapterr(List<Chat> chats) {
        mChats = chats;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (TextUtils.equals(mChats.get(position).sender,
                com.dasta.eemapp.activity.Shop.Home.shop_name)) {
